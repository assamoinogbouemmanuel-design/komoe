...
export default function BudgetDetailPage() {
  ...
  return (
    <div className="animate-in fade-in duration-500 max-w-5xl mx-auto pb-12">
      <Link href="/commune/budget" className="inline-flex items-center text-sm font-bold text-slate-500 hover:text-brand-blue mb-6">
      ...
      <div className="flex justify-between items-start mb-8">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <h2 className="text-3xl font-extrabold text-slate-900 tracking-tight capitalize">Ligne Budgétaire: {id}</h2>
            <Badge variant="success">Actif</Badge>
          </div>
          <p className="text-slate-500 font-medium text-sm">Vue détaillée des fonds et dépenses sur cette ligne précise.</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <Card className="shadow-sm border-slate-100 rounded-3xl bg-brand-blue text-white">
        ...
        <Card className="shadow-sm border-slate-100 rounded-3xl bg-brand-orange text-white">
          <CardContent className="p-6">
        ...
        <Card className="shadow-sm border-slate-100 rounded-3xl bg-slate-50">
          <CardContent className="p-6">
            <p className="text-sm font-bold text-slate-500 uppercase mb-2">Solde Restant</p>
            <p className="text-3xl font-black text-emerald-600">42 500 000</p>
            <p className="text-sm mt-1 text-slate-600">FCFA</p>
          </CardContent>
        </Card>
      </div>
      ...
      <DataTable 
        ...
        data={historiques}
        columns={[
          { header: 'Date', key: 'date', render: (val) => <span className="text-slate-500">{val}</span> },
          ...
          { header: 'Solde après opération', key: 'reste', render: (val) => <span className="font-mono text-slate-900">{val.toLocaleString()} FCFA</span> },
        ]}
      />
    </div>
  );
