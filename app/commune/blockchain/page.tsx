...
const BlockchainMap = dynamic(() => import('@/components/ui/BlockchainMap'), { 
  ssr: false,
  loading: () => <div className="w-full h-[500px] bg-slate-900 rounded-3xl animate-pulse flex items-center justify-center text-slate-500">Chargement de la carte décentralisée...</div>
});
...
export default function BlockchainCommune() {
  return (
    <div className="animate-in fade-in duration-500">
      <div className="mb-8">
        <h2 className="text-2xl font-extrabold text-slate-900 tracking-tight">Réseau Blockchain</h2>
        <p className="text-slate-500 mt-1 font-medium text-sm">État du nœud local et synchronisation avec le réseau Sepolia.</p>
      </div>
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex justify-between items-center p-3 bg-slate-50 rounded-xl">
                <span className="text-sm font-bold text-slate-700">Réseau</span>
                <span className="text-sm font-mono text-emerald-600 bg-emerald-100 px-2 py-1 rounded">Ethereum Sepolia</span>
              </div>
              <div className="flex justify-between items-center p-3 bg-slate-50 rounded-xl">
                <span className="text-sm font-bold text-slate-700">Connexion</span>
              ...
              <div className="flex justify-between items-center p-3 bg-slate-50 rounded-xl">
                <span className="text-sm font-bold text-slate-700">Client Consensus</span>
                <span className="text-sm font-mono text-slate-600">Lighthouse v4.5</span>
              </div>
            </div>
          </CardContent>
        </Card>
              ...
              {[1, 2, 3].map((i) => (
                <div key={i} className="flex justify-between items-center p-3 border border-slate-100 rounded-xl">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-mono font-bold text-slate-900">#45291{88 - i}</p>
                      <p className="text-xs text-slate-500">Il y a {i * 12} secondes</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-xs font-mono text-slate-400">0x8f2a...{Math.random().toString(16).slice(2, 6)}</p>
